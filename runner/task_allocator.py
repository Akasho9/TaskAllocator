"""
Task Allocation Pipeline Runner.
...
from src.task_allocator.task_allocator import (
    DeveloperExpertiseModeler,
    TaskAllocator
)
...
def setup_logger(log_config: Dict[str, Any]) -> logging.Logger:
    ...
    log_file = log_dir / f'task_allocation_{timestamp}.log'

    logger = logging.getLogger('TaskAllocator')
    ...
    logger.info("="*80)
    logger.info("Task Allocation System Initialized")
    logger.info("="*80)
...
def run(data_config: Dict[str, Any], model_config: Dict[str, Any],
    ...
    """Execute the complete task allocation pipeline."""
    logger = setup_logger(log_config)

    logger.info("Phase 1: Data Loading and Validation")
    logger.info("-" * 80)

    expertise_modeler = DeveloperExpertiseModeler(data_config, logger)
    ...
    logger.info("\nPhase 3: Developer Expertise Embedding Generation")
    logger.info("-" * 80)

    expertise_modeler.prepare_technology_mapping(technologies_df)
    expertise_modeler.compute_developer_embeddings()

    logger.info("\nPhase 4: Task Allocation Processing")
    logger.info("-" * 80)

    allocator = TaskAllocator(allocation_config, logger, expertise_modeler)
    allocations = allocator.allocate_all_tasks(tasks_df)

    logger.info("\nPhase 5: Results Generation and Reporting")
    logger.info("-" * 80)
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    report_path = output_dir / f'allocation_report_{timestamp}.csv'

    allocation_report.to_csv(report_path, index=False)
    logger.info(f"Allocation report saved to: {report_path}")

    logger.info("\n" + "="*80)
    logger.info("Task Allocation Pipeline Completed Successfully")
    logger.info("="*80)
...
def main():
    """Entry point for the task allocation runner."""
